...
def projet_Paul_main(classes_to_include,criterion,model,nb_epochs):
    ...
    train_subset = filter_classes_Paul(train_dataset, classes_to_include)
    #test_subset = filter_classes_Paul(test_dataset, classes_to_include)
...
# Choix de la fonction de perte (loss function) et de l'optimiseur
    ...
    return train_losses_Paul,train_accuracies_Paul
def projet_Melvyn_main(classes_to_include,criterion,model,nb_epochs):
    
    train_binary_subset = filter_classes_Benjamin(train_dataset, classes_to_include)
    #test_binary_subset = filter_classes(test_dataset, classes_to_include)
    # Création des DataLoader pour l'entraînement et le test
    train_loader = DataLoader(train_binary_subset, batch_size=64, shuffle=True)
    #test_loader = DataLoader(test_binary_subset, batch_size=batch_size, shuffle=False)
    # Choix de la fonction de perte (loss function) et de l'optimiseur
    model.to(device)
    optimizer = Adam(model.parameters(), lr=1e-3) #Adam(clf.parameters(), lr=1e-3)
    train_losses_Benjamin,train_accuracies_Benjamin=train_model_Benjamin(model, train_loader, optimizer, criterion, nb_epochs)
    return train_losses_Benjamin,train_accuracies_Benjamin
...
def projet_Ethan_main(classes_to_include,criterion,model,nb_epochs):
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=0)
    train_losses_Benjamin,train_accuracies_Benjamin=train_model_Benjamin(model, train_loader, optimizer, criterion, nb_epochs)
    #test_loss, test_acc = test_model_Benjamin(model, test_loader, criterion)
...
train_losses_Paul, train_accuracies_Paul = projet_Paul_main(classes_to_include, criterion, SimpleCNN(10), nb_epochs)
...
train_losses_Hugo, train_accuracies_Hugo = projet_Hugo_main(classes_to_include, criterion,  Net(10), nb_epochs)
...
plot_metrics(data_accuracy_test,data_loss_test,train_losses_Paul, train_accuracies_Paul,train_losses_Benjamin, train_accuracies_Benjamin, train_losses_Melvyn, train_accuracies_Melvyn,train_losses_Hugo, train_accuracies_Hugo,train_losses_Younes, train_accuracies_Younes,train_losses_Ethan, train_accuracies_Ethan)
